...
export class Migration20241122 extends CallableInstance<[string, string, string], Promise<any>> {
    ...
    protected async migrateSignUpsUnder(
        ...
        for await (const documentSnapshot of this.db.collection(`groups/${to.groupId}/meetups/${to.meetupId}/timeslots`).select().stream() as unknown as ReadableStream<DocumentSnapshot<{}>>) {
    ...
    protected async setupPlaylists(oldGroupId: string, newGroupId: string) {
        ...
        for await (const documentSnapshot of stream) {
            ...
            await this.searchClient.partialUpdateObject({ indexName: Indexes.Meetups, objectID: newMeetupId, attributesToUpdate: { playlistId } })
