...
export class Migration20241122 extends CallableInstance<[string, string, string], Promise<any>> {
    ...
    protected async migrateProfiles(administratorId: string, oldGroupId: string, newGroupId: string) {
        ...
        for await (const documentSnapshot of stream) {
            ...
            await this.db.runTransaction(async (transaction) => {
                const createProfile = new SaveProfile({ db: this.db, transaction })
                await createProfile(userId, newGroupId, {
                    name,
                    line,
                    levelId,
                })
            })
            this.logger.info(`createProfile 花費了 ${measureElapsedTime()} ms`)
        }
    }
    ...
    protected async migrateSignUpsUnder(
        ...
        for await (const documentSnapshot of this.db
            .collection(`groups/${to.groupId}/meetups/${to.meetupId}/signUps`)
            .select()
        ...
        for await (const documentSnapshot of stream) {
            ...
            const stauts = this.newStatusesMap[String(oldStatus)]
            if (!stauts) continue
            for (const index of indexes) {
                const timeslotId = TimeslotRepository.nextId({ meetupId: to.meetupId, timestamp: timestamp + index })
                try {
                    if (stauts === SignUpStatuses.Pending) {
                        const measureElapsedTime = createElapsedTimeProfiler()
                        await this.db.runTransaction(async (transaction) => {
                    ...
                    if (stauts === SignUpStatuses.Cancelled) {
                        const measureElapsedTime = createElapsedTimeProfiler()
                        await this.db.runTransaction(async (transaction) => {
                    ...
                    if (stauts === SignUpStatuses.Accepted) {
                        const measureElapsedTime = createElapsedTimeProfiler()
                        await this.db.runTransaction(async (transaction) => {
                    ...
                    if (stauts === SignUpStatuses.Revoked) {
                        const measureElapsedTime = createElapsedTimeProfiler()
                        await this.db.runTransaction(async (transaction) => {
                    ...
                    if (stauts === SignUpStatuses.Refunded) {
                        const measureElapsedTime = createElapsedTimeProfiler()
                        await this.db.runTransaction(async (transaction) => {
    ...
    async execute(userId: string, oldGroupId: string, newGroupId: string) {
        ...
        if (true) await this.migrateSignUps(oldGroupId, newGroupId)
    }
}
